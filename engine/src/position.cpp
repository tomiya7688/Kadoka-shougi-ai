...
PieceType promote_type(PieceType type) {
    ...
    default: throw std::invalid_argument("piece cannot be promoted in SFEN");
    }
}
...
Position Position::from_sfen(std::string_view sfen) {
    ...
    if (ply == 0) {
        throw std::invalid_argument("SFEN ply must be at least 1");
    }
    ...
    for (char ch : board_field) {
        if (ch == '/') {
            if (file != 0 || promoted || rank >= 9) {
                throw std::invalid_argument("invalid SFEN board rank");
            }
            ++rank;
            file = 9;
            continue;
        ...
        if (ch >= '1' && ch <= '9') {
            if (promoted) {
                throw std::invalid_argument("promotion marker before empty squares");
            }
            file -= ch - '0';
            if (file < 0) {
                throw std::invalid_argument("too many files in SFEN rank");
            }
            continue;
        }
        if (ch == '+') {
            if (promoted) {
                throw std::invalid_argument("duplicate SFEN promotion marker");
            }
            promoted = true;
            continue;
        }
        if (file < 1 || rank > 9) {
            throw std::invalid_argument("too many pieces in SFEN board");
        }
    ...
    if (rank != 9 || file != 0 || promoted) {
        throw std::invalid_argument("SFEN board must contain exactly 9 ranks of 9 files");
    }

    if (side_field == "b") {
        position.side_to_move_ = Color::Black;
    } else if (side_field == "w") {
        position.side_to_move_ = Color::White;
    } else {
        throw std::invalid_argument("SFEN side must be b or w");
    }
    ...
    if (hands_field != "-") {
        ...
        for (char ch : hands_field) {
            if (std::isdigit(static_cast<unsigned char>(ch))) {
                ...
                if (count > std::numeric_limits<std::uint8_t>::max()) {
                    throw std::invalid_argument("SFEN hand count is too large");
                }
                continue;
            }
            const PieceType type = piece_type_from_letter(ch);
            if (type == PieceType::King) {
                throw std::invalid_argument("king cannot be in hand");
            }
            ...
            if (static_cast<unsigned>(slot) + actual_count > std::numeric_limits<std::uint8_t>::max()) {
                throw std::invalid_argument("SFEN hand count is too large");
            }
        ...
        if (count != 0) {
            throw std::invalid_argument("SFEN hand count must precede a piece");
        }
    }
